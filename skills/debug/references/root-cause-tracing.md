...
Use the bundled bisection helper: `scripts/find-polluter.sh` (relative to this
skill directory). It runs the test files one-by-one and stops at the first one
that creates the unwanted file or directory.

```bash
# Defaults to `npm test <file>` per test file
./scripts/find-polluter.sh '.git' 'src/**/*.test.ts'

# Or pass a custom per-file test command as a third argument
./scripts/find-polluter.sh '.git' '*.test.js' 'node'
```

If a bundled script cannot run in the current runtime, reproduce the same
bisection inline with native shell: enumerate the test files, run them one at a
time, and check after each whether the pollution path now exists.
