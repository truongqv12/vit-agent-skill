...
- Use a file-discovery/scouting capability to find relevant files whenever context is missing or conflicting; otherwise use native content and name search
- Use a repository-packing capability for a broad packaged snapshot only when it materially improves the investigation; do not generate a mandatory summary document
- Use a documentation-lookup capability for package/plugin documentation; otherwise read the project's own docs and sources
...
**Design targeted, evidence-backed fixes (recommended to the caller; debug does not apply them).**
...
- Switch to `systematic-debugging.md` for the code-level diagnosis
- Use `root-cause-tracing.md` if error is deep in call stack
- Apply `defense-in-depth.md` guidance when recommending the fix
