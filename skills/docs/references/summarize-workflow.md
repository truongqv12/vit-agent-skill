...
3. Use a scouting capability or focused repository search for the requested
   topics when the runtime exposes it and the user's request permits it;
   otherwise search inline with native tools (read, search, shell). Do not scan
   the entire codebase unless the request requires it.
