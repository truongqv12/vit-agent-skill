...
template inherited from any source kit. Read `references/doc-content-rules.md`
before authoring or delegating document changes.
...
- Scout the relevant source and tests. Skip credentials, caches, dependencies,
  generated bundles, and unrelated external trees. Use a scouting capability
  when the runtime exposes it and the user's request permits it; otherwise
  scout inline with native tools (read, search, shell).
...
Use a documentation-authoring capability through the runtime's delegation when
the runtime exposes it and the user's request permits it; otherwise perform the
same evidence-first work inline with native tools (read, search, shell). If
neither path is possible, stop and report the missing capability honestly.
Never emit a named source-subagent call as the only path.

Pass the brainstormed contract, discovered routes, evidence map, exact files to
retain, replace, or remove, and relevant rules from
