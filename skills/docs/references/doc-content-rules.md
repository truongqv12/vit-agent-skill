...
relevant ownership, drift-resistance, and authority rules in every delegated
authoring prompt because delegated contexts are isolated.
...
4. Distill current rationale into the repository's canonical decision surface.
   Remove obsolete or superseded prose; version-control history preserves the
   long form.
