---
name: brainstorm
description: "Turn unclear intent into an accepted outcome and compare viable approaches before delivery. Use at the opening of multi-step work or when a diagnosed problem has meaningful solution choices."
user-invocable: true
...
to the next owning workflow. Resolve the owner from capabilities available in
the current runtime instead of assuming a particular CLI or skill collection:

- Feature or documentation delivery: use an available planning capability,
  followed by the appropriate implementation capability. If neither is
  installed, create and execute a scoped native plan.
- Diagnosed bug: use an available debugging or repair capability. If none is
  installed, hand off the expected behavior, root-cause evidence, and chosen
  cause-aligned repair to the runtime's native workflow.
...
**Typically precedes:** planning or implementation work available in the current
runtime.

**Bug path:** opening intent frame -> scout and diagnose -> solution brainstorm
when needed -> cause-aligned repair.
