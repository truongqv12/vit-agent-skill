...
- Pre-landing review (from a ship/release pipeline)
- Explicit request for checklist review
- Security audit before release
- Reviewing significant changes (10+ files or security-sensitive)
...
- These block the ship/release pipeline
...
For each critical issue, ask the user (use an interactive prompt capability when
available; otherwise ask in conversation):
...
## Integration with a ship/release pipeline

A ship/release pipeline calls this workflow before landing. Critical findings
block the pipeline. Informational findings are included in the PR body.

## Integration with the standard review pipeline

When invoked as part of standard code review, the checklist augments (not
replaces) the existing scout → review → fix → verify pipeline. Checklist
findings are merged with the reviewer's own findings.
