...
description: Use after implementation, before code review to proactively find edge cases, side effects, and potential issues via a scouting pass - catches problems a quality review might miss
---
...
Use a **scouting** capability when the runtime exposes it and the user permits
it; otherwise perform the scouting inline with native search and scoped reads.
Scouting is read-only.

## Purpose

Code reviews catch obvious issues but miss subtle side effects. Scouting detects:
- Files affected by changes a reviewer might not check
...
Scout for edge cases in the recent changes. Provide the changed files and ask
for:
...
## What Scouting Catches

| Issue | Why Missed | Scouting Detects |
|-------|------------|------------------|
