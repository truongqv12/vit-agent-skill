...
Resolve the review arguments into a diff for the review pipeline.
...
| *(none + no context)* | Prompt | ask the user |

## Resolution Commands

### PR Mode (explicit-intent and optional)

PR mode uses a GitHub CLI (`gh`) capability. Use it only when the user asked to
review a PR **and** `gh` is available and authenticated.
...
**Native fallback (no `gh`):** fetch the PR head branch if needed, then review
with git directly:

```bash
git fetch origin "pull/$PR_NUM/head:pr-$PR_NUM"   # if the branch is not local
git diff "$(git merge-base <base> pr-$PR_NUM)"...pr-$PR_NUM
git diff <base>...pr-$PR_NUM --name-only
```

**Context passed to the review:**
- PR title and description (intent)
- Base branch (what it merges into)
- Full diff
- Changed file list for scouting
...
When no arguments and no recent context, ask the user what to review (use an
interactive prompt capability when available; otherwise ask in conversation):
- Header: "Review Target"
- Question: "What would you like to review?"
- Options: Pending changes, Enter PR number, Enter commit hash, Full codebase scan, Parallel codebase audit

For PR/commit options, follow up to get the number/hash.
...
| Error | Action |
|-------|--------|
| PR not found | `gh pr view` (or the fetch) fails → report "PR #N not found in this repo" |
