...
Merge a GitHub pull request via `gh`, then watch post-merge CI on the target
branch until green and verify the result before stopping. `gh` is required for
this workflow — check `gh auth status` first, and merge only on explicit user
intent.
...
3. Apply the fix: use a fix/debug capability when the runtime exposes one and
   the user permits it, passing the exact failing command and error evidence;
   otherwise diagnose and fix inline with native tools. If neither path is
   possible, stop and report the failure honestly.
