...
2. If impact exists, route the update through a documentation capability when the
   runtime exposes one, passing it the changed contract, the evidence, and the
   exact docs in scope. Otherwise, edit the smallest owning doc inline with
   native tools.
